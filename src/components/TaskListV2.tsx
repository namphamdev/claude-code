...
export function TaskListV2({ tasks, isStandalone = false }: Props): React.ReactNode {
  ...
  if (previousCompletedIdsRef.current === null) {
    previousCompletedIdsRef.current = new Set<string>();
    // ⚡ Bolt: Use a single for loop instead of .filter().map() to avoid array allocations and GC pressure
    for (let i = 0; i < tasks.length; i++) {
      if (tasks[i].status === 'completed') {
        previousCompletedIdsRef.current.add(tasks[i].id);
      }
    }
  }
