...
export function TaskListV2({
  ...
  if (previousCompletedIdsRef.current === null) {
    // ⚡ Bolt Optimization: Using a single for-loop directly populating a Set
    // instead of chained .filter().map() avoids allocating and garbage-collecting
    // intermediate arrays on every render loop.
    const initialCompletedIds = new Set<string>()
    for (const t of tasks) {
      if (t.status === 'completed') {
        initialCompletedIds.add(t.id)
      }
    }
    previousCompletedIdsRef.current = initialCompletedIds
  }
  ...
  // ⚡ Bolt Optimization: Using a single for-loop directly populating a Set
  // instead of chained .filter().map() reduces GC pressure.
