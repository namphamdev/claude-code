...
export function TaskListV2({
  ...
  if (previousCompletedIdsRef.current === null) {
    ...
    for (let i = 0; i < tasks.length; i++) {
      if (tasks[i].status === 'completed') initialCompletedIds.add(tasks[i].id)
    }
  ...
  for (let i = 0; i < tasks.length; i++) {
    if (tasks[i].status === 'completed') currentCompletedIds.add(tasks[i].id)
  }
  const now = Date.now()
  ...
  for (let i = 0; i < tasks.length; i++) {
    if (tasks[i].status !== 'completed') unresolvedTaskIds.add(tasks[i].id)
  }
